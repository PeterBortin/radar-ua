...
(function(){
  ...
  function buildRun(SETUP,seed){
    ...
    SETUP.threats.slice(0,CAP).forEach(function(st){var w=WP[st.model];if(!w)return;var total=pathLen(st.path);
      if(st.path.length<2||total<2)return;
      var tt=buildTT(w,total), dur=tt[tt.length-1].t;
      threats.push({id:st.id,model:st.model,w:w,cat:w.cat,path:st.path,total:total,tt:tt,dur:dur,launchT:st.launchT||0,targetName:nearestCity(st.path[st.path.length-1]),outcome:null});});
    var sams=SETUP.pvo.map(function(pv){var s=PVO[pv.key];
      return {id:pv.id,key:pv.key,name:s.name,pos:pv.pos,r:s.r,mSpeed:s.mSpeed,ammo:s.ammo,cr:s.cr,bal:s.bal,dr:s.dr,
        ignoreCheap:s.ignoreCheap,ceil:PVO_CEIL[s.grp]||1e9,gun:s.r<=5,ammoLeft:s.ammo,engaged:new Set(),pac3:pv.key==="patriot"?6:0,pac2:pv.key==="patriot"?10:0};});
  ...
  function stateAt(RUN,t){
    var counts={drone:0,cruise:0,ballistic:0,hyper:0},nK=0,nI=0,air=[],fly=[],fx=[];
    RUN.threats.forEach(function(th){var res=th.outcome;
      if(t>=res.t){if(res.kind==="kill")nK++;else if(res.kind==="impact"&&th.w.strike)nI++;return;}
    ...
    RUN.events.forEach(function(ev){if((ev.kind==="kill"||ev.kind==="impact")&&Math.abs(t-ev.t)<0.8&&ev.pos)fx.push({kind:ev.kind,pos:ev.pos,a:1-Math.abs(t-ev.t)/0.8});});
    return {counts:counts,nK:nK,nI:nI,air:air,fly:fly,fx:fx};
  }
  ...
  window.Delta9Sim = { WP:WP, PVO:PVO, UA:UA, SP:SP, CATUK:CATUK, buildRun:buildRun, stateAt:stateAt, demoZNPP:demoZNPP, catColor:catColor, hav:hav, pathLen:pathLen, posOnPath:posOnPath, altAt:altAt, phaseAt:phaseAt };
})();
